...
if (isset($_POST['btnEdit'])) {
    if ($permissions['products']['update'] == 1) {
        ...
        if (!empty($name) && !empty($category_id) &&  !empty($serve_for) && !empty($description) && empty($error['cancelable'])) {
            if (strpos($name, "'") !== false) {
            ...
            if (!empty($image)) {
                ...
                $sql_query = "UPDATE products SET name = '$name' ,slug = '$slug' , subcategory_id = '$subcategory_id', image = '$upload_image', description = '$description', indicator = '$indicator', manufacturer = '$manufacturer', made_in = '$made_in', return_status = '$return_status', cancelable_status = '$cancelable_status', till_status = '$till_status' WHERE id = $ID";
                $db->sql($sql_query);
                
            } else {
                $sql_query = "UPDATE products SET name = '$name' ,slug = '$slug' ,category_id = '$category_id' ,subcategory_id = '$subcategory_id' ,description = '$description', indicator = '$indicator', manufacturer = '$manufacturer', made_in = '$made_in', return_status = '$return_status', cancelable_status = '$cancelable_status', till_status = '$till_status' WHERE id = $ID";
                $db->sql($sql_query);
            }
            // echo $sql_query;
            $res = $db->getResult();
