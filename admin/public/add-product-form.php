...
if (isset($_POST['btnAdd'])) {
    if ($permissions['products']['create'] == 1) {
        ...
        $manufacturer = $db->escapeString($fn->xss_clean($_POST['manufacturer']));
        $made_in = $db->escapeString($fn->xss_clean($_POST['made_in']));
        $return_status = $db->escapeString($fn->xss_clean($_POST['return_status']));
        $cancelable_status = $db->escapeString($fn->xss_clean($_POST['cancelable_status']));
        $till_status = $db->escapeString($fn->xss_clean($_POST['till_status']));
        ...
        if ($_FILES["other_images"]["error"][0] == 0) {
            for ($i = 0; $i < count($_FILES["other_images"]["name"]); $i++) {
                ...
                if ($_FILES["other_images"]["error"][$i] > 0) {
                    ...
                    !(($_FILES["other_images"]["type"][$i] == "image/gif") ||
        ...
        if (!empty($name) && !empty($category_id) && !empty($serve_for) && empty($error['other_images']) && empty($error['cancelable']) && !empty($description)) {
            ...
            $sql = "INSERT INTO products (name,slug,category_id,subcategory_id,image,other_images,description,indicator,manufacturer,made_in,return_status,cancelable_status, till_status) VALUES('$name','$slug','$category_id','$subcategory_id','$upload_image','$other_images','$description','$indicator','$manufacturer','$made_in','$return_status','$cancelable_status','$till_status')";
            // echo $sql;
            $db->sql($sql);
                    ...
                    <div class="box-body">
                        ...
                        <div id="packate_div" style="display:none">
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group packate_div">
                                        <label for="price">Price (<?= $settings['currency'] ?>):</label> <i class="text-danger asterik">*</i><input type="text" class="form-control" name="packate_price[]" id="packate_price" required />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group packate_div">
                                ...
                                <div class="col-md-1">
                                    <div class="form-group packate_div">
                                        ...
                                        <input type="text" class="form-control" name="packate_stock[]" required="" />
                                    </div>
                                </div>
                                <div class="col-md-1">
                                    <div class="form-group packate_div">
                        ...
                        <div class="form-group">
                            <label for="subcategory_id">Sub Category :</label> <i class="text-danger asterik">*</i><?php echo isset($error['subcategory_id']) ? $error['subcategory_id'] : ''; ?>
