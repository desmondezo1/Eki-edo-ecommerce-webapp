...
if (isset($_GET['table']) && $_GET['table'] == 'products') {
	...
	$sql = "SELECT p.id AS id, p.name, p.image, p.indicator, p.manufacturer, p.made_in, p.return_status, p.cancelable_status, p.till_status,p.description, pv.id as product_variant_id, pv.price, pv.discounted_price, pv.measurement, pv.serve_for, pv.stock,pv.stock_unit_id, u.short_code 
            FROM `products` p
            $join 
...
// 	echo $sql;
	...
	foreach ($res as $row) {
		...
		$operate .= ' <a class="btn-xs btn-danger" href="delete-product.php?id=' . $row['product_variant_id'] . '" title="Delete"><i class="fa fa-trash-o"></i></a>';
...
if (isset($_GET['table']) && $_GET['table'] == 'withdrawal-requests') {
	...
	if (isset($_GET['search']) && $_GET['search'] != '') {
		$search = $db->escapeString($fn->xss_clean($_GET['search']));
		$where = " Where `id` like '%" . $search . "%' OR `type_id` like '%" . $search . "%' OR `type` like '%" . $search . "%' OR `status` like '%" . $search . "%' OR `date_created` like '%" . $search . "%' OR `last_updated` like '%" . $search . "%'";
	}
	...
	if (isset($_GET['type']) && $_GET['type'] != '' && $_GET['type'] == 'user') {
		...
		$sql = "SELECT * FROM `withdrawal_requests`" . $where;
	}
	// echo $sql;
	$db->sql($sql);
	$res = $db->getResult();
...
// 	echo $sql;
	...
	foreach ($res as $row) {
		...
		$tempRow['action'] = '<a href="order-detail.php?id=' . $row['order_id'] . '" title="View Order"><i class="fa fa-folder-open"></i>&nbsp;&nbsp;</a> <a href="invoice.php?id=' . $row['order_id'] . '"><i class="fa fa-eye" title="View Invoice"></i></a>';
