...
if ((isset($_POST['get_addresses'])) && ($_POST['get_addresses'] == 1)) {
    ...
    if (!empty($user_id)) {
        if ($fn->is_address_exists($id = "", $user_id)) {
            ...
            $db->sql($sql);
            $total = $db->getResult();
            $sql = "select u.*,c.name as city_name,a.name as area_name from user_addresses u LEFT JOIN city c ON c.id=u.city_id LEFT JOIN area a ON a.id=u.area_id where u.user_id=" . $user_id . " ORDER BY is_default DESC";
            $db->sql($sql);
            $res = $db->getResult();
            if (!empty($res)) {
                $address['error'] = false;
