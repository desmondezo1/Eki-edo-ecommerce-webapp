		...
		if ($num_rows > 0) {
			...
			if (isset($_FILES['profile']) && !empty($_FILES['profile']) && $_FILES['profile']['error'] == 0 && $_FILES['profile']['size'] > 0) {
				...
				if (!(in_array($extension, $allowedExts))) {
					$response["error"]   = true;
					$response["message"] = "Profile type is invalid!";
					echo json_encode($response);
					return false;
				}
			...
			$sql = "select c.name as city_name, a.name as area_name from city c left join area a on c.id=a.city_id where c.id = " . $city . " AND a.id = " . $area;
			$db->sql($sql);
			$city_area = $db->getResult();
			$area_name = !empty($city_area) ? $city_area[0]['area_name'] : "";
			$city_name = !empty($city_area) ? $city_area[0]['city_name'] : "";
