...
if ((isset($_POST['get_user_cart'])) && ($_POST['get_user_cart'] == 1)) {
    $user_id  = (isset($_POST['user_id']) && !empty($_POST['user_id'])) ? $db->escapeString($fn->xss_clean_array($_POST['user_id'])) : "";
    $limit = (isset($_POST['limit']) && !empty($_POST['limit']) && is_numeric($_POST['limit'])) ? $db->escapeString($fn->xss_clean_array($_POST['limit'])) : 10;
    $offset = (isset($_POST['offset']) && !empty($_POST['offset']) && is_numeric($_POST['offset'])) ? $db->escapeString($fn->xss_clean_array($_POST['offset'])) : 0;
    if (!empty($user_id)) {
        if ($fn->is_item_available_in_user_cart($user_id)) {
            ...
            $db->sql($sql);
            $total = $db->getResult();
            $sql = "select * from cart where user_id=" . $user_id . " ORDER BY date_created DESC LIMIT $offset,$limit";
            $db->sql($sql);
            $res = $db->getResult();
            $i = 0;
            $j = 0;
            ...
            foreach ($res as $row) {
                ...
                $sql = "select pv.*,p.name,p.image,p.other_images,pv.measurement,(select short_code from unit u where u.id=pv.measurement_unit_id) as unit from product_variant pv left join products p on p.id=pv.product_id where pv.id=" . $row['product_variant_id'];
                $db->sql($sql);
                ...
                for ($k = 0; $k < count($res[$i]['item']); $k++) {
                    ...
                    // $price = $res[$i]['item'][$k]['discounted_price'] == 0 ? $res[$i]['item'][$k]['price'] * $row['qty'] : $res[$i]['item'][$k]['discounted_price'] * $row['qty'];
