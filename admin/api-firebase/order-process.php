...
// 	}
    ...
    $sql = "INSERT INTO `orders`(`user_id`,  `mobile`, `total`, `delivery_charge`, `tax_amount`, `tax_percentage`, `wallet_balance`, `discount`, `promo_code`, `promo_discount`, `final_total`, `payment_method`, `address`, `latitude`, `longitude`, `delivery_time`, `status`, `active_status`) VALUES ('$user_id','$mobile','$total','$delivery_charge','$tax_amount','$tax_percentage','$walletvalue','$discount','$promo_code','$promo_discount','$final_total','$payment_method','$address','$latitude','$longitude','$delivery_time','$order_status','received')";
    // echo $sql;
    $db->sql($sql);
...
// 		$db->insert('order_items', $data);
        ...
        $sql = "INSERT INTO `order_items`(`user_id`, `order_id`, `product_variant_id`, `quantity`, `price`, `discounted_price`, `discount`, `sub_total`, `status`, `active_status`) VALUES ('$user_id','$neworder_id','$product_variant_id','$quantity','$order_price','$discounteds_price','$discount','$order_sub_total','$order_item_status','received')";
        // echo $sql;
        // echo $quantity;
        $db->sql($sql);
		$res = $db->getResult();
	...
	if (!empty($result)) {
		...
		if ($postStatus == 'cancelled') {
			...
			if ($cancelled == $total) {
				...
				// $order_status = $db->escapeString(json_encode($status));
				// $sql = "UPDATE orders SET status = '".$order_status."',active_status = '".$currentStatus."' WHERE id=".$order_id;
				// $db->sql($sql);
			}
		...
		if ($postStatus == 'returned') {
			...
			// check if user already applied for return request
	...
	if (!empty($res)) {
		...
		if ($postStatus == 'cancelled' || $postStatus == 'returned') {
			...
			if ($res[0]['payment_method'] != 'cod' && $res[0]['payment_method'] != 'COD') {
				...
				$function->update_wallet_balance($new_balance, $user_id);
				/* add wallet transaction */
				$wallet_txn_id = $function->add_wallet_transaction($id, $user_id, 'credit', $sub_total, 'Balance credited against item cancellation.');
			} else {
